...
mtcars %>%
  ggplot(aes(x = wt , y = mpg , fill= factor(cyl))) +
  geom_point(shape = 21 , size= 4, alpha = 0.6)+
  geom_text(aes(label = (factor(cyl)), size=3 ,vjust=-1))
...
mtcars %>%
  ...
  # A�adir capa de texto con label = rownames(mtcars) y color rojo
   geom_text(aes(label = rownames(mtcars)) , color= 'green', size=3 ,vjust=-1)))
...
# codigo anterior
  ...
  palette <- c("0" = "green", "1" = "blue")
mtcars %>%
  ggplot(data=., aes(cyl, fill = factor(am))) +
  geom_bar() +labs(x = "Numeros de cilindros",
                   y = "Recuento", fill= 'transmision') +
...
mtcars %>%
  ggplot(data=., aes(cyl, fill = factor(am))) +
  geom_bar(position = "dodge") +labs(x = "Numeros de cilindros",
                   y = "Recuento", fill= 'transmision') + 
  scale_fill_manual(values = palette)
# geometrias ----
# geom_*
...
# plot de lineas ----
btcusd <- read_csv("~/EstComputacional/visualizacion/BTC-USD.csv", 
